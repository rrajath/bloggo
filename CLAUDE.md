...
- **Minification:** the `release` build type runs R8 (`isMinifyEnabled = true`, `isShrinkResources = true`), which brings the APK from ~31 MB to ~5 MB. Keep rules live in `app/proguard-rules.pro` (GitHub DTO serializers, a kotlinx.serialization backstop, and `-dontwarn com.google.errorprone.annotations.**` for Tink). Retrofit, OkHttp, Room, Coil, and the Kotlin serialization plugin supply their own rules. Smoke-test a release build after touching dependencies or the DTOs, since debug builds are not minified.
