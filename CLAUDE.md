...
- **Release shrinking:** `isMinifyEnabled` + `isShrinkResources` are on for the `release` build type (R8 + resource shrinking). This cut the release APK from ~50MB to ~7MB — almost all of the original size was unstripped dex from `material-icons-extended` and the rest of the dependency graph. Debug builds are unaffected (`isMinifyEnabled = false`). `app/proguard-rules.pro` carries manual keep/dontwarn rules for the libraries that ship as plain jars rather than AARs (Moshi codegen adapters, Retrofit/OkHttp, SnakeYAML) plus `androidx.security.crypto`'s Tink dependency (references compile-only `errorprone` annotations that R8 otherwise reports as missing). Room, Hilt, and Sentry bring their own consumer rules via their AARs and need nothing extra. If R8 fails with "Missing classes," check `app/build/outputs/mapping/release/missing_rules.txt` for the exact rule to add.
